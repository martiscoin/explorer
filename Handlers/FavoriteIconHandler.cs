...
namespace Marscore.Explorer.Handlers
{
   public class FavoriteIconHandler
   {
      ...
      public async Task Invoke(HttpContext context)
      {
         ...
         byte[] bytes = await http.GetByteArrayAsync(settings.Icon).ConfigureAwait(true);
         await context.Response.Body.WriteAsync(bytes, 0, bytes.Length).ConfigureAwait(true);
      }
   }
}
